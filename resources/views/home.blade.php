...
<form  role="form" method="POST" action="{{ url('recherche') }}">
    ...
    <div class="form-group{{ $errors->has('recherche') ? ' has-error' : '' }}">
       <div class="input-group">
        <input placeholder="Recherche par nom,  id,  etc..."id="name" type="text" class="form-control" name="recherche" value="{{ old('name') }}" autofocus>
...
<p>Résultats pour <em>{{$chaine}}</em>:</p>
    ...
    @else
        ...
        <a class="list-group-item" href="{{url('dossiers/'.$result->id.'/show')}}">{{$result->prenom.' '.$result->nom}}</a>
        @endforeach
        </div>
    @endif
@endif
